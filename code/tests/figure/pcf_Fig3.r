...
f0=read.table('../simulation/pcf_dpow0p25_area1_tmax100_N1000_U0p0.txt',sep=";",header=T,dec=".")
f0$rDelta=f0$r/Delta
f_ab0=read.table("../simulation/nb_individuals_dpow0p25_area1_tmax100_N1000_U0p0.txt",sep=";",header=T,dec=".")

plot(0.1,0.1,t="n",xlim=range(f0$rDelta),ylim=c(0.01,max(f0$pcf_dx)),log="xy",xlab=expression(r/Delta),ylab="g(r,t)-1",axes=F,cex.lab=1.25)

axis(1, at=log10Tck('x','major'), tcl= 0.2,cex.axis=1.25) # bottom
axis(2, at=log10Tck('y','major'), tcl= 0.2,cex.axis=1.25) # left
...
lines(f0$rDelta,G_theoretical(0.0,f0$rDelta,f_ab0$Nb_ind/f_ab0$area,"0",100),col="black",lty=2,lwd=2)

f1=read.table('../simulation/pcf_dpow0p25_area1_tmax100_N1000_U0p1.txt',sep=";",header=T,dec=".")
f1$rDelta=f1$r/Delta
f_ab1=read.table("../simulation/nb_individuals_dpow0p25_area1_tmax100_N1000_U0p1.txt",sep=";",header=T,dec=".")

lines(f1$rDelta,f1$pcf_dx-1,lty=1,col="blue",lwd=2)
#lines(f1$rDelta,f1$pcf_dp-1,lty=1,col="cyan",lwd=2)
lines(f1$rDelta,G_theoretical(0.026,f1$rDelta,f_ab1$Nb_ind/f_ab1$area,"1",100),col="blue",lty=2,lwd=2)

f2=read.table('../simulation/pcf_dpow0p25_area1_tmax100_N1000_U0p5.txt',sep=";",header=T,dec=".")
f2$rDelta=f2$r/Delta
f_ab2=read.table("../simulation/nb_individuals_dpow0p25_area1_tmax100_N1000_U0p5.txt",sep=";",header=T,dec=".")

lines(f2$rDelta,f2$pcf_dx-1,lty=1,col="red",lwd=2)
#lines(f2$rDelta,f2$pcf_dp-1,lty=1,col="pink",lwd=2)
lines(f2$rDelta,G_theoretical(0.51,f2$rDelta,f_ab2$Nb_ind/f_ab2$area,"1",100),col="red",lty=2,lwd=2)

f3=read.table('../simulation/pcf_dpow0p25_area1_tmax100_N1000_U2p5.txt',sep=";",header=T,dec=".")
f3$rDelta=f3$r/Delta
f_ab3=read.table("../simulation/nb_individuals_dpow0p25_area1_tmax100_N1000_U2p5.txt",sep=";",header=T,dec=".")

lines(f3$rDelta,f3$pcf_dx-1,lty=1,col="darkgreen",lwd=2)
#lines(f3$rDelta,f3$pcf_dp-1,lty=1,col="green",lwd=2)
lines(f3$rDelta,G_theoretical(2.4,f3$rDelta,f_ab3$Nb_ind/f_ab3$area,"1",100),col="darkgreen",lty=2,lwd=2)
...
##Zoom in
plot(0,0,t="n",xlim=c(0,20),ylim=c(0.,3*10^9),xlab=expression(r/Delta),ylab=expression(paste("g(r,t)-1 (x 10"^"9",")")),yaxt="n",cex.lab=1.25,cex.axis=1.25)
axis(2,at=c(0,10^9,2*10^9,3*10^9),labels=c(0,1,2,3),cex.axis=1.25)
